...
Use `latest` for normal installs. Use `dev` while testing changes before they are promoted to `main`. Dev builds may use prerelease versions such as `1.1.0-dev.12`; stable releases use normal version tags such as `1.1.7`. The image tag chooses the branch/release track; no separate runtime `GIT_BRANCH` setting is needed.

For the exact v1.1.7 release, pin one of these tags:

```text
ghcr.io/thedinz/spotifybu:v1.1.7
ghcr.io/thedinz/spotifybu:1.1.7
